...
func createQueryForMultipleInsertVacancies(vacancies []models.Vacancy) (query string, valArgs []interface{}) {
	...
	for _, v := range vacancies {
		valStrings = append(valStrings, fmt.Sprintf("($%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d)", valInsertCount, valInsertCount+1, valInsertCount+2, valInsertCount+3, valInsertCount+4, valInsertCount+5, valInsertCount+6, valInsertCount+7, valInsertCount+8, valInsertCount+9, valInsertCount+10, valInsertCount+11))
		valArgs = append(valArgs,  v.Id, v.Title, v.Url, 0, v.PositionId, v.Areas, v.Specs, v.Experience, v.SalaryFrom, v.SalaryTo, v.Skills, v.Date)
		valInsertCount += 12
	}
	query = `INSERT INTO vacancy (id, name, url, city_id, position_id, prof_areas, specs, experience, salary_from, salary_to, key_skills, vacancy_date) 
